...
We have, 
$$ f(z_0)^{(n)}  = \frac{n!}{2n\pi} \int_C \frac{dz}{z^{n+1}} $$ 
So $\int_C \frac{dz}{z^{n+1}} = 0 $
\begin{ex}
   $$ \frac{z}{2z + 1}\: dz$$
\end{ex}
   $= -\pi i$
...
\begin{ex}
   $$ \frac{e^{-z}}{(z - 5)(z+ 2)}$$
$=0$ because the doesn't exist a singularity within the contour hence the integral is 0 as the function is analytic within.
\end{ex}
